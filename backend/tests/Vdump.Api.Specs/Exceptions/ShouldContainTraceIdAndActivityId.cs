...
namespace Vdump.Api.Specs.Exceptions {
  ...
  public sealed class ShouldContainTraceIdAndActivityId : IClassFixture<WebApplicationFactory<Startup>> {
    private readonly WebApplicationFactory<Startup> _applicationFactory;

    public ShouldContainTraceIdAndActivityId(WebApplicationFactory<Startup> applicationFactory) {
    ...
    public async Task WhenGeneralErrorOccured() {
      ...
      mappedResponse!.TraceId.Should().MatchRegex("[^[\\d,A-Z]*$");
      mappedResponse!.ActivityId.Should().MatchRegex("^[\\d,\\-,a-z]*$");
    }
